#!/bin/bash
# Test tldraw API with dynamic user prompt
# Usage: ./test-tldraw-prompt.sh "your prompt here"
...
# Use Python to properly build the JSON payload
python3 - <<'PYTHON_SCRIPT' | curl -X POST "$TLDRAW_URL" \
  -H "Content-Type: application/json" \
  -H "Accept: */*" \
  -H "Origin: https://agent.templates.tldraw.dev" \
  -H "Referer: https://agent.templates.tldraw.dev/" \
  --no-buffer \
  -d @-
import json
import sys
