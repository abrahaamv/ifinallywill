...
[
    {
        ...
        title: `<h2 class='document-header'>Enduring Power of Attorney for Property of {{personal.fullName}}</h2><br />`,
        order: 1,
        fallback: "",
        content: `<p><strong>ENDURING POWER OF ATTORNEY FOR PROPERTY OF {{personal.fullName}}</strong></p>`,
...
{{#if personal.province}}, {{personal.province}}{{/if}}, revoke any previous Enduring Power of Attorney for
    ...
    {{#if @index}}
      <p>
        {{#with (lookup ../attorneyTwo (subtract @index 1))}}
          If {{fullName}} cannot or will not be my Attorney because of refusal, resignation, death, mental incapacity, or
        {{/with}}
        removal by the court, I SUBSTITUTE {{fullName}}
        {{#if city}} of {{city}}{{/if}}
        {{#if province}}, {{province}}{{/if}}{{#if country}}, {{country}}{{/if}} to be my sole Attorney(s).
...
<ul>
  <li>"Act" means the Alberta Powers of Attorney Act, R.S.A. 2000, c. P-20.</li>
...
<p>This document shall be considered to be an Enduring Power of Attorney for Property under the Act.</p>
<p>I revoke any previous Enduring Powers of Attorney for Property.</p>
...
{{else}}
  <p>No conditions or restrictions upon Enduring Power of Attorney.</p>
{{/if}}`,
  ...
  <p>The authority granted to my Attorney under this Enduring Power of Attorney for Property will be in effect if and
...
{{#if_eq activationType "immediate"}}
  <p>The authority granted to my Attorney under this Enduring Power of Attorney for Property will be in effect immediately.</p>
{{/if_eq}}`,
...
<p>SIGNED AND DECLARED by {{personal.fullName}} on this ____ day of ____________________, 20____to be the
  Grantor's Enduring Power of Attorney for Property,  who at the Grantor's request and in the
  presence of the Grantor,  and in the physical presence of each other at _______________________,
  all being present at the same time, have signed our names as witnesses in the Grantor's presence
