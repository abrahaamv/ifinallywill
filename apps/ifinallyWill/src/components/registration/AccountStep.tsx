...
export function AccountStep({ data, onUpdate, onNext, onBack }: Props) {
  ...
  const canContinue = isGoogleUser
    ? !!data.email
    : !!data.email && !!data.password && data.password.length >= 8 && !!data.password_confirmation && passwordsMatch;
  ...
  return (
    <div className="epilogue-step-card animate-slide-in-right">
      ...
      <div className="epilogue-form-container">
          ...
          {!isGoogleUser && (
            <>
              ...
              <FloatingInput
                ...
                onChange={(v) => onUpdate({ password_confirmation: v })}
                type={showPassword ? 'text' : 'password'}
                autoComplete="new-password"
                error={
        ...
        {!isGoogleUser && (
          <>
            <div style={{ textAlign: 'center', margin: '1.5rem 0', color: 'var(--color-text-secondary)', fontWeight: 600 }}>
              or
            </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '0.5rem', color: '#FFBF00', fontSize: '1.25rem' }}>
            &#9733;&#9733;&#9733;&#9733;&#9733;
          </div>
          <p style={{ fontSize: '1rem', color: '#000', fontWeight: 500 }}>Rated 4.8 stars on Google</p>
        </div>
      </div>
