...
export function FinalDetailsStep({ estateDocId, willData, onNext, onPrev, isFirstStep, isLastStep }: StepProps) {
    ...
    >
      <div className="space-y-4">
        <div className="ifw-info-box">
          <strong>Important:</strong> Your will must be signed in the presence of two witnesses who are{' '}
          <em>not</em> beneficiaries of your will.
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="fd-w1" className="block text-sm font-medium mb-1">Witness 1 Name</label>
            <input
              id="fd-w1"
              {...register('witnessOne')}
              className="ifw-input"
            ...
            />
            {errors.witnessOne && <p className="text-xs text-[var(--ifw-error)] mt-1">{errors.witnessOne.message}</p>}
          </div>
          <div>
            <label htmlFor="fd-w2" className="block text-sm font-medium mb-1">Witness 2 Name</label>
            <input
              id="fd-w2"
              {...register('witnessTwo')}
              className="ifw-input"
              placeholder="Full legal name"
            />
            {errors.witnessTwo && <p className="text-xs text-[var(--ifw-error)] mt-1">{errors.witnessTwo.message}</p>}
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="fd-loc" className="block text-sm font-medium mb-1">Signing Location</label>
            <input
              id="fd-loc"
          ...
          <div>
            <label htmlFor="fd-date" className="block text-sm font-medium mb-1">Signing Date</label>
            <input
              id="fd-date"
              type="date"
