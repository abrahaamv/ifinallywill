...
import { authMiddleware, requireRole, AuthError, logoutCleanup } from '../src/lib/middleware';
import { sql } from '@platform/db';
import { TEST_TENANT_IDS, createMockSession, createMockRequest } from './helpers';
...
describe('authMiddleware()', () => {
	...
	it('should extract auth context and set RLS tenant ID', async () => {
		...
		vi.mocked(tenantContextModule.extractTenantFromSession).mockReturnValue(TEST_TENANT_IDS.tenant1);
	...
	it('should throw INVALID_TENANT error when tenant ID format is invalid', async () => {
		...
		vi.mocked(authModule.auth).mockResolvedValue(mockSession);
		vi.mocked(tenantContextModule.extractTenantFromSession).mockReturnValue(TEST_TENANT_IDS.invalidUuid);
	...
	it('should enforce role hierarchy: owner > admin > member', async () => {
		...
		for (const testCase of testCases) {
			...
			if (testCase.shouldPass) {
				const context = await requireRole(req, testCase.requiredRole as 'owner' | 'admin' | 'member');
				expect(context.role).toBe(testCase.userRole);
			} else {
				await expect(requireRole(req, testCase.requiredRole as 'owner' | 'admin' | 'member')).rejects.toThrow(
					AuthError,
				);
			}
		}
